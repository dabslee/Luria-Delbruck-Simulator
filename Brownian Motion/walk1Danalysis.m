% Runs 100 simulations of 1D walks of 1000 steps each, then does the
% following:
%  (1) Calculates the mean final displacement and mean final squared
%      displacement
%  (2) Plots the first 10 random walks as a function of the number of steps
%  (3) Plots the mean displacement as a function of the number of steps
%  (4) Plots the mean squared displacement as a function of the number of
%      steps
% It prints (1) to the command window, while saving (2)-(4) as images.
...
for i = 1:simulations
    xs(i,:) = walk1D(startpos, steps, stepsize);
end
...
% Plot the first 10 random walks as a function of the number of steps
bound = max([50,max(xs)]);
for i = 1:10
    plot(0:steps,xs(i,:),'LineWidth',1);
    format(gcf,"Random Walk "+num2str(i),...
        "Step #","Position (x)",[0,steps],[-bound,bound]);
    saveas(gcf,"plots/randomwalk"+num2str(i)+".png");
end
